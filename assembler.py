             ...
             if(self.__islabel(self.__asm[i][0])) :
                lc= int(self.__asm[i][1],16)
                 ...
                 return
                    ...
                    lc+=1
                    instructions = self.__asm[i][1] if self.__islabel(self.__asm[i][0]) else self.__asm[i][0]
                    bit_15= '0'
                    
                    if instructions in self.__mri_table.keys() :
                        address=str(self.__address_symbol_table[self.__asm[i][1]])
                        opcode = str(self.__mri_table[instructions])
                        self.__bin[self.__format2bin(str(lc)),'dec',12)] = bit_15 + opcode + address
                        lc+=1
                        elif instructions in self.__rri_table.keys() :
                        self.__bin[self.__format2bin(str(lc)),'dec',12)] = self.__rri_table[instructions]
                    lc+=1
             elif instructions in self.__ioi_table.keys() :
                        self.__bin[self.__format2bin(str(lc)),'dec',12)] = self.__ioi_table[instructions]
              lc+=1
